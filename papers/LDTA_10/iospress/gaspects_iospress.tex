...
\usepackage{times}%
\usepackage[footnotesize,bf,labelsep=period]{caption}
\normalfont%
...
\lstdefinelanguage{Haskell}
    {
      basicstyle=\small\it\rmfamily,%\ttfamily,
...
For example, consider the following pattern: \lstinline!$a=? : $b=$a!. %$
In abstract syntax it is written as follows:
\mbox{$P = \abstr{a}{\;?SymRef}{\abstr{b}{a}{Rule(a, b)}}$}. 
Projections for $P$ are \mbox{$P_a = \abstr{a}{\;?SymRef}{Rule(a, a)}$} and \mbox{$P_b = \abstr{b}{\;?SymRef}{Rule(?SymRef, b)}$}.

\figref{Eq} defines a function $\Eq{}$ that takes a variable and a pattern as arguments and returns a system of set inequalities. We denote variables $v$ which are supposed to be fresh with respect to the rest of the system as $\fresh{v}$. For example, usage of this notation in the first rule in \figref{Eq} means that fresh variables $\fresh{v_i}$ are introduced every time this rule is applied.

\begin{Def}[Matching]\label{SPT}
Given a pointcut expression $P$, a GT $t$ and $R \in \mathcal{V} \setminus \mathcal{V}_P$, let 
$$S^R_P(t) = \{R = t\} \cup \bigcup\limits_{v \in \mathcal{V}_P} \Eq{}(R, P_v)$$
Then, $P$ \emph{matches} $t$ iff there is a solution $\sigma$ to $S^R_P(t)$.
\end{Def}
...
\framebox{$
\begin{array}{rcl}%
\Eq(V, \mathbf{Type}(t_1, \ldots, t_n)) &=& \{V \subseteq \mathbf{Type}(\fresh{v_1}, \ldots, \fresh{v_n})\} \cup 
\bigcup_1^n \Eq{}(\fresh{v_i}, t_i),\\%
\mbox{where } \mathbf{Type} &\subseteq& \{
	Grammar, Rule, Iter, SymRef, Literal, Empty\}\\%
%
\Eq{}(V, ?Type) &=& \{V \subseteq \sem{Type}\}\\%
%
\Eq{}(V, Seq(e_1, \ldots, e_n)) &=& \{V \subseteq Seq(\fresh{L})\} \cup \{ \fresh{L} = \fresh{L_1} \oplus \ldots \oplus \fresh{L_n}\} \cup \bigcup_1^n \Eq{}(\fresh{L_i}, e_i)\\%
%
\Eq{}(V, [?]) &=& \{ V \subseteq \sem{List(Expr)} \}\\%
%
\Eq{}(V, Alt(e_1, \ldots, e_n, \{?\})) &=& \{V \subseteq Alt(\fresh{S})\}
                                   \cup \bigcup_1^n \Eq{}(\fresh{t_i}, e_i) \cup                                 
                                    \bigcup_1^n \{ \fresh{t_i} \subseteq \fresh{S}\}
...
\Eq{}(V, \abstr{v}{e_1}{e_2}) &=& \Eq{}(v, e_1) \cup \Eq{}(V, e_2) 
        \cup \{varax(v)\},\\ %
        && \mbox{where } varax(v) = \forall x', x'' \in v. Strip(x') = Strip(x'')\\%
%
\Eq{}(V, v) &=& \{V \subseteq v\}%
\end{array}
$}
...
Our approach is implemented as an open-source tool \Grammatic{}%\footnote{The tool is available at \url{http://grammatic.googlecode.com}}
